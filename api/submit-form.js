export default async function handler(req, res) {
  ...
  const FORM_ID      = '9zbiM3NTf3iQSpMFnUIn'
  ...
  try {
    const r = await fetch('https://services.leadconnectorhq.com/contacts/', {
      ...
      body: JSON.stringify({
        ...
        source: 'VirtuPro Growth Roadmap Lead Magnet',
        tags: ['lead-magnet', 'growth-roadmap'],
  ...
  if (contactId && CG6GeFiqxFrpb5m9YVMl) {
    try {
      const r = await fetch(`https://services.leadconnectorhq.com/contacts/${contactId}/notes`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ body: `Source: VirtuPro Growth Roadmap Lead Magnet\nProperties managed: ${CG6GeFiqxFrpb5m9YVMl}` }),
      })
      console.log('[roadmap] note →', r.status)
    } catch (err) {
      console.error('[roadmap] note threw:', err.message)
    }
  }
